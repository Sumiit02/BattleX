...
def _is_admin_logged_in():
    return bool(session.get('user')) and session.get('role') == 'admin'


def _get_admin_preview_username():
    if _is_admin_logged_in():
        return (session.get('admin_preview_user') or '').strip()
    return ''


def _is_admin_player_preview_active():
    return bool(_get_admin_preview_username())


def _get_player_context_username():
    if bool(session.get('user')) and session.get('role') == 'player':
        return session.get('user')
    preview_user = _get_admin_preview_username()
    if preview_user:
        return preview_user
    return None


def _can_access_player_portal():
    return bool(_get_player_context_username())
...
def _freefire_mode_label(mode_key):
    for key, label in FREEFIRE_PRIZE_MODES:
        if key == mode_key:
            return label
    return mode_key or ''


def _normalize_freefire_mode_key(value):
    if not value:
        return None
    text = str(value).strip().lower()
    compact = ''.join(ch for ch in text if ch.isalnum())

    if 'br' in compact or 'battleroyale' in compact:
        if 'solo' in compact:
            return 'br_solo'
        if 'duo' in compact or '2v2' in compact:
            return 'br_duo'
        if 'squad' in compact or 'team' in compact or '4v4' in compact:
            return 'br_squad'
    if 'cs' in compact or 'clashsquad' in compact:
        if '1v1' in compact:
            return 'cs_1v1'
        if '2v2' in compact:
            return 'cs_2v2'
...
def init_db():
    ...
    if USE_POSTGRES:
        ...
        cur.execute("""
        CREATE TABLE IF NOT EXISTS prize_money_requests (
            id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        """)
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN IF NOT EXISTS winner_note TEXT")
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN IF NOT EXISTS admin_note TEXT")
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN IF NOT EXISTS requested_by TEXT")
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN IF NOT EXISTS approved_by TEXT")
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN IF NOT EXISTS approved_at TIMESTAMP")
    ...
    """)
    try:
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN winner_note TEXT")
    except Exception:
        pass
    try:
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN admin_note TEXT")
    except Exception:
        pass
    try:
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN requested_by TEXT")
    except Exception:
        pass
    try:
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN approved_by TEXT")
    except Exception:
        pass
    try:
        cur.execute("ALTER TABLE prize_money_requests ADD COLUMN approved_at TEXT")
    except Exception:
        pass
...
def home():
    ...
    if session.get('role') == 'admin' and not _is_admin_player_preview_active():
...
def wallet_deposit():
    ...
    if _is_admin_player_preview_active():
        return jsonify({'success': False, 'error': 'Player preview mode is read-only for wallet actions.'}), 403
...
def wallet_cashfree_callback():
    if not _can_access_player_portal():
        ...
        return redirect(url_for('login'))
    if _is_admin_player_preview_active():
        flash('Player preview mode is read-only for wallet actions.', 'error')
...
def player_settings():
    ...
    if request.method == 'POST':
        if _is_admin_player_preview_active():
            conn.close()
            flash('Player preview mode is read-only for settings changes.', 'error')
...
def players():
    ...
    return render_template('players.html', players=all_players, user=username, role='player')
...
def register_freefire():
    ...
    if request.method == 'POST':
        if _is_admin_player_preview_active():
            flash('Player preview mode is read-only for registrations.', 'error')
            return redirect(url_for('register_freefire', mode=selected_mode, event_id=event_id))
        flash("Please complete the payment to finish registration.", "info")
        return redirect(url_for('register_freefire'))

    wallet_balance_paise = _wallet_user_balance_paise_by_name(username)
...
def proceed_registration():
    ...
    if _is_admin_player_preview_active():
        return jsonify({'success': False, 'error': 'Player preview mode is read-only for registrations.'}), 403
    ...
    if not primary_phone:
        ...
        try:
            ...
            lookup_cur.execute('SELECT phone FROM users WHERE username = ?', (username,))
    ...
    try:
        ...
        try:
            conn = sqlite3.connect(DB_NAME)
            cur = conn.cursor()
            msg = f"Pending registration saved: reg id {reg_id} by {username}"
            metadata = json.dumps({'registration_id': reg_id, 'username': username, 'mode': data.get('mode')})
...
def wallet_pay_registration():
    if not _can_access_player_portal():
        return jsonify({'success': False, 'error': 'Authentication required'}), 403
    if _is_admin_player_preview_active():
        return jsonify({'success': False, 'error': 'Player preview mode is read-only for wallet payments.'}), 403
...
def admin_payments():
    ...
    cur = conn.cursor()
    cur.execute('''
        SELECT id, username, email, game_id, phone, mode, team_size, payment_id, order_id, amount,
               status, refund_requested, refunded, event_id, created_at,
               manual_upi, manual_payer_name, manual_txn_id, manual_paid_at
        FROM registrations
    ...
    for r in rows:
        regs.append({
            ...
            'payment_id': r[7], 'order_id': r[8], 'amount': r[9], 'status': r[10], 'refund_requested': bool(r[11]),
            'refunded': bool(r[12]), 'event_id': r[13], 'created_at': r[14],
            'manual_upi': r[15], 'manual_payer_name': r[16], 'manual_txn_id': r[17], 'manual_paid_at': r[18]
        })
    conn.close()
...
def admin_wallet():
    ...
    for row in prize_rows:
        mode_key = row[4] or ''
        if mode_key not in groups_by_key:
            normalized = _normalize_freefire_mode_key(row[5])
            mode_key = normalized if normalized in groups_by_key else mode_key
        if mode_key not in groups_by_key:
            continue
        groups_by_key[mode_key]['requests'].append({
            'id': row[0],
            'event_id': row[1],
            ...
            'mode_label': row[5],
            'amount': row[6],
            'amount_label': _format_currency_value(row[6]),
            'status': row[7] or 'pending',
            'winner_note': row[8] or '',
            'admin_note': row[9] or '',
...
@app.route('/admin/wallet/prize-requests/<int:request_id>/decision', methods=['POST'])
def admin_wallet_prize_request_decision(request_id):
    if 'user' not in session or session.get('role') != 'admin':
        flash('Admins only', 'error')
        return redirect(url_for('admin_login'))
    ...
    try:
        cur.execute(
            '''SELECT id, event_id, registration_id, username, amount, status, mode_label
               FROM prize_money_requests
               WHERE id = ?''',
            (request_id,)
        )
        ...
        if action == 'approve':
            cur.execute('UPDATE users SET wallet_balance = COALESCE(wallet_balance, 0) + ? WHERE username = ?', (amount_paise, username))
            cur.execute(
                '''UPDATE prize_money_requests
                   SET status = ?, admin_note = ?, approved_by = ?, approved_at = ?
                   WHERE id = ?''',
                ('approved', admin_note, session.get('user'), datetime.utcnow().isoformat(), request_id)
            )
            cur.execute(
                'INSERT INTO wallet_transactions (username, txn_type, amount, status, note) VALUES (?, ?, ?, ?, ?)',
                (
                    ...
                    amount_paise,
                    'completed',
                    f'Prize request #{request_id} approved for event #{event_id}, registration #{registration_id}'
                )
            )
            cur.execute(
                'INSERT INTO notifications (type, message, metadata) VALUES (?, ?, ?)',
                (
                    'wallet_winnings_approved',
                    f'Prize money approved for {username}',
                    json.dumps({'prize_request_id': request_id, 'username': username, 'amount_paise': amount_paise, 'event_id': event_id, 'registration_id': registration_id, 'mode': mode_label})
                )
            )
            conn.commit()
            flash(f'Prize approved and credited to {username}: {_format_currency_value(amount_paise)}', 'success')
        else:
            cur.execute(
                '''UPDATE prize_money_requests
                   SET status = ?, admin_note = ?, approved_by = ?, approved_at = ?
                   WHERE id = ?''',
                ('rejected', admin_note, session.get('user'), datetime.utcnow().isoformat(), request_id)
            )
            cur.execute(
                'INSERT INTO notifications (type, message, metadata) VALUES (?, ?, ?)',
                (
                    'wallet_winnings_rejected',
                    f'Prize money request rejected for {username}',
                    json.dumps({'prize_request_id': request_id, 'username': username, 'event_id': event_id, 'registration_id': registration_id, 'mode': mode_label})
                )
            )
            conn.commit()
            flash('Prize request rejected.', 'success')
    except Exception:
        conn.rollback()
        flash('Failed to process prize request.', 'error')
    finally:
        conn.close()
...
@app.route('/admin/player-portal')
def admin_player_portal():
    ...
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute('SELECT username, email, game_id, phone FROM users WHERE role = ? ORDER BY username', ('player',))
    rows = cur.fetchall()
    conn.close()
    ...
    return render_template(
        'admin_player_portal.html',
        players=players,
        preview_user=_get_admin_preview_username()
    )


@app.route('/admin/player-portal/activate', methods=['POST'])
def admin_player_portal_activate():
    if not _is_admin_logged_in():
        flash('Admins only', 'error')
        return redirect(url_for('admin_login'))
    ...
    if not username:
        flash('Select a player to preview.', 'error')
        return redirect(url_for('admin_player_portal'))

    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute('SELECT username FROM users WHERE username = ? AND role = ?', (username, 'player'))
    row = cur.fetchone()
    conn.close()
    if not row:
        flash('Player account not found.', 'error')
        return redirect(url_for('admin_player_portal'))

    session['admin_preview_user'] = username
    flash(f'Player preview activated for {username}.', 'success')
    return redirect(url_for('admin_player_portal'))


@app.route('/admin/player-portal/clear', methods=['POST'])
def admin_player_portal_clear():
    if not _is_admin_logged_in():
        flash('Admins only', 'error')
        return redirect(url_for('admin_login'))

    session.pop('admin_preview_user', None)
    flash('Player preview stopped.', 'success')
    return redirect(url_for('admin_player_portal'))
...
def _fetch_event_registration_payload(event_id):
    ...
    try:
        ...
        prize_requests_by_reg = {rid: [] for rid in reg_ids}
        if reg_ids:
            ...
            try:
                cur.execute(f"""
                    SELECT id, registration_id, amount, status, mode_label, winner_note, admin_note, created_at, approved_at
                    FROM prize_money_requests
                    WHERE registration_id IN ({placeholders})
                    ORDER BY id DESC
                """, reg_ids)
                for prize_request in cur.fetchall():
                    prize_requests_by_reg[prize_request['registration_id']].append({
                        'id': prize_request['id'],
                        'amount_label': _format_currency_value(prize_request['amount']),
                        'status': prize_request['status'] or 'pending',
                        'mode_label': prize_request['mode_label'] or '',
                        'winner_note': prize_request['winner_note'] or '',
                        'admin_note': prize_request['admin_note'] or '',
                        'created_at': prize_request['created_at'] or '',
                        'approved_at': prize_request['approved_at'] or ''
                    })
            except Exception:
                pass
    except LookupError:
    ...
    for row in registration_rows:
        ...
        registrations_payload.append({
            ...
            'players': players,
            'prize_requests': prize_requests_by_reg.get(row['id'], [])
        })
...
def admin_credit_winnings(event_id, registration_id):
    ...
    amount_paise = _wallet_rupees_to_paise(amount_rupees)
    winner_note = (request.form.get('winner_note') or '').strip()
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    try:
        cur.execute(
            '''SELECT r.username, r.mode, r.status, e.mode
               FROM registrations r
               LEFT JOIN events e ON e.id = r.event_id
        ...
        username, reg_mode, reg_status, event_mode = row[0], row[1], row[2], row[3]
        if str(reg_status or '').lower() != 'completed':
            conn.close()
            flash('Prize request can be created only for paid/completed registrations.', 'error')
            return redirect(url_for('admin_match_detail', event_id=event_id))

        cur.execute(
            'SELECT id FROM prize_money_requests WHERE registration_id = ? AND status = ? ORDER BY id DESC LIMIT 1',
            (registration_id, 'pending')
        )
        existing_pending = cur.fetchone()
        if existing_pending:
            conn.close()
            flash('A pending prize request already exists for this registration.', 'info')
            return redirect(url_for('admin_match_detail', event_id=event_id))

        mode_key = _normalize_freefire_mode_key(reg_mode) or _normalize_freefire_mode_key(event_mode)
        if not mode_key:
            conn.close()
            flash('Event mode is not recognized for prize routing. Use one of the 7 Free Fire modes.', 'error')
            return redirect(url_for('admin_match_detail', event_id=event_id))
        mode_label = _freefire_mode_label(mode_key)

        cur.execute(
            '''INSERT INTO prize_money_requests
               (event_id, registration_id, username, mode_key, mode_label, amount, status, winner_note, requested_by)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (event_id, registration_id, username, mode_key, mode_label, amount_paise, 'pending', winner_note, session.get('user'))
        )
        request_id = cur.lastrowid
        cur.execute(
            'INSERT INTO notifications (type, message, metadata) VALUES (?, ?, ?)',
            (
                'wallet_winnings_pending',
                f'Prize request #{request_id} created for {username}',
                json.dumps({'prize_request_id': request_id, 'event_id': event_id, 'registration_id': registration_id, 'amount_paise': amount_paise, 'mode': mode_label})
            )
        )
        conn.commit()
        flash(f'Prize request submitted for approval: {mode_label} • ₹{amount_rupees}', 'success')
    except Exception:
        conn.rollback()
        flash('Failed to create prize request.', 'error')
    finally:
        conn.close()
