...
def proceed_registration():
    ...
    def is_upi(v):
        if not v:
            return False
        return bool(re.match(r"^[A-Za-z0-9._-]{2,}@[A-Za-z0-9]{2,}$", str(v).strip()))
    ...
    payout_upi = (data.get('payout_upi') or '').strip().lower()
    if not is_upi(payout_upi):
        return jsonify({'success': False, 'error': 'Invalid or missing UPI ID'}), 400
